...
- ただし互換ヒントとして次のみ使用:
  - `alt`
  - iReal Pro direct-export の `suspended-fourth` で表現される sus 系 (`7sus4`, `9sus4`, `7b9sus4`)
...
- `7#9b5`: `add#9 + alter b5` を保持
- `maj7#5`: `major-seventh + alter5=+1` として構造保持
- `7#5b9`: `alter5=+1` + `add9=-1` を保持
- `7b5b9`: `alter5=-1` + `add9=-1` を保持
- `13`, `13b9`: `dominant-13th` と `alter9=-1` を区別保持
- `maj9`: `major-ninth` を `maj9` に正規化
- `mMaj7`:
  - direct: `minor + add7=+1`
  - converted: `major-minor`
  - 正規化: `mMaj7`
- sus 系:
  - direct `suspended-fourth` は `kind@text` と degree を互換ヒントとして評価
  - `7sus4` / `9sus4` / `7b9sus4` を区別正規化
  - converted 側の `dominant(+ninth) + add4 + subtract3` と同一セマンティクスへ統一
...
未対応の `kind` は暗黙に `7` へフォールバックしない。
`UnsupportedMusicXMLHarmonyError` を明示的に送出する。
...
- `source_position_quarters` は保持する
  ...
  - `offset` は current musical location からの相対値として、`cursor + offset/divisions` で記録
